    ...
    <my-dialog :is-show="isShowPayDialog"
      ...
      <table class="buy-dialog-table">
        ...
        <tr>
          ...
          <td>{{ period.label }}</td>
          <td>
            <span v-for="item in versions" :key="item.label">
              {{ item.label }}
            </span>
          </td>
          <td>{{ price }}</td>
...
export default {
  ...
  methods: {
    ...
    getPrice () {
      ...
      //通过vue-resource 发送Ajax请求 与后端确定好接口（json文件中getPrice）
    ...
    confirmBuy () {
      ...
      //通过vue-resource 发送Ajax请求 与后端确定好接口（json文件中createOrder）
  ...
  mounted () {
    //组件渲染完成之后，第一次进入页面时执行的业务逻辑
    this.buyNum = 1
    this.buyType = this.buyTypes[0]
    this.versions = [this.versionList[0]]
    this.period = this.periodList[0]
    this.orderId = "6djk979"
    this.getPrice()
  }
}
</script>
